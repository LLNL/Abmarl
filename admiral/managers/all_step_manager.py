...
class AllStepManager(SimulationManager):
    ...
    def step(self, action_dict, **kwargs):
        ...
        obs = {
            agent.id: self.env.get_obs(agent.id) for agent in self.agents.values()
            if agent.id not in self.done_agents
        }
        rewards = {
            agent.id: self.env.get_reward(agent.id) for agent in self.agents.values()
            if agent.id not in self.done_agents
        }
        dones = {
            agent.id: self.env.get_done(agent.id) for agent in self.agents.values()
            if agent.id not in self.done_agents
        }
        infos = {
            agent.id: self.env.get_info(agent.id) for agent in self.agents.values()
            if agent.id not in self.done_agents
        }
