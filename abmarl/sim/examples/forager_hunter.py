...
class FoodAgent(ComponentAgent):
    def __init__(self, **kwargs):
        kwargs['team'] = 1
        super().__init__(**kwargs)
...
class HuntingForagingSim(AgentBasedSimulation):
    ...
    def __init__(self, num_food, **kwargs):
        ...
        kwargs['agents'] = {**kwargs['agents'], **food}
        # Explicitly pull out the the dictionary of agents. This makes the env
        # easier to work with.
        self.agents = kwargs['agents']

        # Hack the team attack matrix and number of teams
        ...
        kwargs['team_attack_matrix'] = team_attack_matrix
        kwargs['number_of_teams'] = 3
        ...
        for agent in self.agents.values():
            if isinstance(agent, (Forager, Hunter)):
    ...
    def step(self, action_dict, **kwargs):
        ...
        for agent_id, action in action_dict.items():
            ...
            proposed_amount_move = action.get('move', np.zeros(2, dtype=np.int))
