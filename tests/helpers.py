...
class MultiAgentGymSpacesEnv(MultiAgentEnv):
    def __init__(self):
        ...
        self.agents = {
            'agent0': Agent(id='agent0', observation_space=MultiBinary(4), action_space=Tuple((Dict({'first': Discrete(4), 'second': Box(low=-1, high=3, shape=(2,), dtype=np.int)}), MultiBinary(3)))),
            'agent1': Agent(id='agent1', observation_space=Box(low=0, high=1, shape=(1,), dtype=np.int), action_space=MultiDiscrete([4, 6, 2])),
