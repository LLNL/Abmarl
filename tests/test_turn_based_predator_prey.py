...
def test_turn_based_predator_prey_distance():
    ...
    env.agents['prey3'].position = np.array([2, 3])
    env.agents['prey4'].position = np.array([3, 3])
    env.agents['prey5'].position = np.array([3, 1])
    env.agents['prey6'].position = np.array([2, 1])
    obs = {'predator0': env.env.get_obs('predator0')}
    ...
    obs, reward, done, info = env.step({'prey1': np.array([0, -1])})
    ...
    obs, reward, done, info = env.step({agent_id: np.array([1, 1]) for agent_id in obs})
    ...
    obs, reward, done, info = env.step({'prey4': np.array([-1, 1])})
    ...
    obs, reward, done, info = env.step({agent_id: np.array([0, 0]) for agent_id in obs})
    np.testing.assert_array_equal(obs['predator0']['predator1'], np.array([-2, -2,  2]))
    ...
    obs, reward, done, info = env.step({agent_id: np.array([-1, 0]) for agent_id in obs})
    ...
    obs, reward, done, info = env.step({'prey5': np.array([-1, 0])})
    np.testing.assert_array_equal(obs['prey6']['predator0'], np.array([0, 2,  2]))
    ...
    obs, reward, done, info = env.step({agent_id: np.array([0, -1]) for agent_id in obs})
    ...
    obs, reward, done, info = env.step({'predator0': {'attack': 1, 'move': np.array([0, 0])}})
    np.testing.assert_array_equal(obs['predator1']['predator0'], np.array([1, 2,  2]))
    np.testing.assert_array_equal(obs['predator1']['prey0'], np.array([0, 0,  0]))
...
def test_turn_based_predator_prey_grid():
    ...
    env_config = {
        'region': 6,
        'observation_mode': PredatorPreyEnv.ObservationMode.GRID,
        'agents': agents,
    }
    env = PredatorPreyEnv.build(env_config)
    env = TurnBasedManager(env)
