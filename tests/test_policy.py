...
def test_epsilon_soft_policy_init_epsilon():
    ...
    with pytest.raises(AssertionError):
        policy = EpsilonSoftPolicy(table, epsilon=-0.2)
    with pytest.raises(AssertionError):
        policy = EpsilonSoftPolicy(table, epsilon=1.4)
...
def test_epsilon_soft_policy_probability():
    ...
    policy = EpsilonSoftPolicy(table, epsilon=0.5)
    prob = policy.probability(4, 2)
    assert prob == (1 - policy.epsilon + policy.epsilon / policy.q_table[4].size if 2 == np.argmax(policy.q_table[4]) else policy.epsilon / policy.q_table[4].size)
...
def test_random_first_action_policy_reset():
    ...
    policy = RandomFirstActionPolicy(table)
    policy.reset()
    assert policy.take_random_action == True
...
def test_random_first_action_policy_act():
    ...
    policy.reset()
    policy.act(1)
    assert policy.take_random_action == False
    action = policy.act(2)
